...
class Dp_Agg(nn.Module):
    ...
    def forward(self, loc_emb, fake_loc, real_loc):
        # TODO: optimization can be considered here, simu the noise add to a location.
        ...
        for usr in real_loc:
            ...
            for loc in real_loc[usr]:
                loc_noise[loc] += noise[i]/loc_cnt[loc] # TODO: noise need to normalize.
                i += 1
            for loc in fake_loc[usr]:
                loc_noise[loc] += noise[i]/loc_cnt[loc]
                i += 1
